...
# Di sinilah semua "keajaiban" styling terjadi.
# Saya menambahkan komentar untuk setiap bagian agar Anda mudah memodifikasinya.
st.markdown("""
<style>
/* --- FONT & LATAR BELAKANG --- */
...
/* Menghapus padding ekstra yang dibuat Streamlit di dalam kolom */
div[data-testid="stVerticalBlock"] > [data-testid="stHorizontalBlock"] > div[data-testid="column"]:nth-child(1) {
    /* Menargetkan hanya kolom pertama (kiri) */
    padding-left: 0;
}
div[data-testid="stVerticalBlock"] > [data-testid="stHorizontalBlock"] > div[data-testid="column"]:nth-child(2) {
    /* Menargetkan hanya kolom kedua (kanan) */
    padding-right: 0;
}

/* --- TOMBOL & INPUT MODERN (DIPERBAIKI) --- */
/* Menggunakan st.form akan membuat layout tombol dan input lebih rapi */
.stForm {
...
.leaderboard-entry .rank { font-weight: 700; color: #00C9A7; /* Warna untuk rank */ }
</style>
""", unsafe_allow_html=True)

# --- 2. INISIALISASI STATE & MODEL ---
...
# --- 3. FUNGSI LOGIKA INTI (DIPERBAIKI) ---
...
def calculate_metrics(points):
    ...
    golden_ratio_score = face_height / face_width if face_width > 0 else 0
    return symmetry_score, golden_ratio_score

def get_dynamic_messages(symmetry_score, golden_ratio_score):
    """Membuat narasi dinamis yang menarik."""
    if symmetry_score < 1.5:
        sym_title = "Level Dewa! ✨"
        sym_text = "Struktur wajahmu menunjukkan tingkat simetri yang luar biasa tinggi. Ini adalah kualitas langka yang sering diasosiasikan dengan aura keren yang universal."
...
def analyze_and_draw(image):
    """Fungsi tunggal untuk menganalisis dan menggambar, menjamin konsistensi."""
...
class VideoProcessor(VideoTransformerBase):
    ...
    def recv(self, frame):
        ...
        # --- LOGIKA UNTUK LIVE DETECTION (HANYA VISUAL, TANPA SKOR) ---
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        faces = face_detector.detectMultiScale(gray, 1.1, 5, minSize=(100, 100))
        for (x, y, w, h) in faces:
            cv2.rectangle(img, (x, y), (x + w, y + h), (0, 255, 0, 0.5), 2) # Kotak hijau transparan
            rect = dlib.rectangle(int(x), int(y), int(x + w), int(y + h))
            landmarks = landmark_predictor(gray, rect)
            points = [landmarks.part(i) for i in range(68)]
            for pt in points:
                cv2.circle(img, (pt.x, pt.y), 2, (255, 255, 0), -1) # Titik kuning
...
with col1:
    ...
    with st.form(key="analysis_form"):
        # PERBAIKAN: Input dan Tombol dalam kolom agar sejajar
        ...
        if analyze_button:
            if not username:
                ...
                with ctx.video_processor.lock: frame = ctx.video_processor.latest_frame
...
with col2:
    default_tab_index = 1 if st.session_state.active_tab == "Analisis" else 0
    ...
    with tab1:
        ...
        if not st.session_state.leaderboard:
            ...
            for i, d in enumerate(st.session_state.leaderboard[:10]):
                st.markdown(f"<div class='leaderboard-entry'><span class='rank'>#{i+1}</span> <span>{d['username']}</span> <span>{d['score']:.2f}</span></div>", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if result and result["face_found"]:
            ...
            with img_col:
                st.image(result["image"], channels="BGR", caption="Hasil Deteksi", width=250)
            with metric_col:
